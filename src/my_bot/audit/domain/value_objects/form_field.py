# my_bot_project/src/my_bot/domain/value_objects/form_field.py
"""
ارزش‌مقدار فیلد فرم (Form Field Value Object).

این کلاس نمایانگر یک فیلد در فرم پویا است که توسط ادمین تعریف می‌شود.
هر فیلد دارای نوع، نام، برچسب، اعتبارسنجی‌ها، گزینه‌ها (برای فیلدهای انتخابی)
و سایر ویژگی‌های مرتبط است.
فیلد فرم به‌صورت غیرقابل تغییر (Immutable) ذخیره می‌شود.
"""
...
class FieldType:
    """
    انواع فیلدهای قابل استفاده در فرم.
...
class FormField:
    ...
    این کلاس یک فیلد را در فرم پویا تعریف می‌کند و قوانین اعتبارسنجی،
    نوع، گزینه‌ها و سایر ویژگی‌های آن را نگهداری می‌کند.
        ...
        if self.default_value is not None:
            ...
            if self.field_type in FieldType.SELECTION_TYPES:
                if self.default_value not in [opt["value"] for opt in self.options]:
                    # اگر چند انتخابی باشد، لیست را بررسی می‌کنیم
    ...
    def _validate_selection(self, value: Any) -> Optional[str]:
        ...
        if self.field_type == FieldType.MULTI_SELECT or self.field_type == FieldType.CHECKBOX:
            # چند انتخابی
    ...
    def get_default_display(self) -> Any:
        ...
        if self.field_type in FieldType.SELECTION_TYPES:
            if self.field_type == FieldType.MULTI_SELECT:
                if isinstance(self.default_value, list):
                    return [opt.get("label", val) for val in self.default_value
                            for opt in self.options if opt.get("value") == val]
            else:
                for opt in self.options:
    ...
    def __str__(self) -> str:
        ...
        return f"FormField(name={self.name}, label={self.label}, type={self.field_type})"